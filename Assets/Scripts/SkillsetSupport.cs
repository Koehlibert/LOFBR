...
public class SkillsetSupport : Skillset
{
    void OnEnable()
    {
        ...
        startingArmor = 8;
        startingSpeed = 14;
        primary = FindAnyObjectByType<ShootHeal>();
        primary.manaCost = 5;
        primary.reloadtime = .8f;
        secondary = FindAnyObjectByType<ShootPoison>();
        secondary.manaCost = 5;
        secondary.reloadtime = 1.2f;
        alternative = FindAnyObjectByType<ManaDrain>();
        alternative.manaCost = 15;
        alternative.reloadtime = 20f;
        skill = FindAnyObjectByType<UltAttackHeal>();
        skill.manaCost = 200;
        skill.reloadtime = 20f;
        ultimate = FindAnyObjectByType<UltRez>();
